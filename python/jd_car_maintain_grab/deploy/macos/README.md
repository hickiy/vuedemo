...
用 LaunchAgent 每天 09:50 拉起抢购，并用 `caffeinate` 保证「等到 09:59:55 再开抢」
...
| 09:59:55 | 发出第一个兑换请求（`ADVANCE_SEC=5`） |
| ~10:00:15 | 跑满 `API_ATTEMPTS=20` 次后退出，并关闭 Chrome；`caffeinate` 释放锁 |
...
（登录态失效）、`等待到 09:59:55 提前开抢`（时序正常）、每条接口返回的业务码。
...
- **时区/时钟**：`--start` 用的是本机本地时间，不在东八区要换算；提前 5 秒开抢要求
  时钟准确，请开启自动设置时间（NTP）。
